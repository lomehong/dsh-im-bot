...
![手机连接配置页](assets/settings-preview.png)
...
| `@dsh-extra/dsh-client-ui-settings-im` | 客户端插件：「插件」页 im-channel 条目的配置区「手机连接」（二维码、绑定管理、访客权限） |
...
1. 打开网页 → 「插件」管理页 → **im-channel** 条目 → 配置（手机连接）。
2. 接入渠道（三选一或多选）：
...
- **访客权限**：Owner 在「插件」页 → im-channel → 配置 →「访客权限」中配置访客可用的**工具**（默认全部禁用，纯对话）与**命令**（默认 帮助/状态/回复/停止/全文）；工具白名单支持精确名与前缀通配（如 `fs*`、`mcp__wecom*`），保存后下一轮对话即生效
...
- **点击展开为 Tab 化状态卡**：头部是微信/飞书/企业微信三个 Tab（图标 + 平台名 + 状态点，
  记住上次选中），主体显示选中平台的：
  - **机器人本体行**：账号标识（微信 accountId / 飞书 appId / 企微 botId）、在线状态、状态点；
  - **绑定明细列表**：该平台全部绑定用户（Owner 在前），每行 = Owner👑/访客徽标 +
    脱敏用户标识（前 8 位）+ 相对绑定时间（如「3 分钟前」）；
- **数据口径**：`configured` = 本地凭证存在；`online` = 路由器当前持有该平台通道实例
  （连接循环运行中，断线自动重连）；数据 30 秒轮询，页面隐藏时暂停；
- **避让**：打开工具详情栏时状态栏自动左移贴其左缘，关闭后回到视口右缘。

对应后端端点：`GET /im-channel/bots/status`（含各平台绑定明细，用户标识已脱敏）。
