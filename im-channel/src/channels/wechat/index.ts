...
export class WechatChannel implements ImChannel {
  ...
  private readonly contextTokenActivity = new Map<string, number>()
  ...
  private static readonly CONTEXT_TOKEN_LIMIT = 5_000
  /** Drop context tokens for users idle longer than this. */
  private static readonly CONTEXT_TOKEN_TTL_MS = 30 * 60_000
  /** Redelivery-window length for fingerprints; entries older than this are pruned. */
  private static readonly FINGERPRINT_WINDOW_MS = 30_000
  ...
  private pruneStale(): void {
    ...
    for (const [key, ts] of this.recentFingerprints) {
      if (now - ts > WechatChannel.FINGERPRINT_WINDOW_MS) this.recentFingerprints.delete(key)
    }
  }
