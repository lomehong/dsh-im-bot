...
import { McpClient } from './wecom/mcp-client.ts'
...
export class McpManagerError extends Error {
  constructor(message: string, readonly code: 'invalid-url' | 'duplicate') {
    super(message)
...
function pushCandidate(out: McpImportResult, name: string, item: Record<string, unknown>): void {
  ...
  if (typeof item.url === 'string' && item.url.trim() !== '') {
    try {
      ...
      if (out.candidates.some(c => c.url === url)) return // 同批 URL 去重
    ...
    for (const line of trimmed.split(/\r?\n/)) {
      ...
      if (HTTP_URL_RE.test(value)) {
        try {
          ...
          if (out.candidates.some(c => c.url === url)) continue
 ...
 */
export async function testMcpServer(url: string, timeoutMs = 8000): Promise<McpTestResult> {
