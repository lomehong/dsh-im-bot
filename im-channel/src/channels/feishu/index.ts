...
export class FeishuChannel implements ImChannel {
  ...
  private async sendContent(target: ReplyTarget, message: OutboundMessage): Promise<void> {
    const client = this.client
    if (client === undefined) throw new Error('飞书通道未连接')
  ...
  private async dispatchSend(client: Lark.Client, target: ReplyTarget, body: { msg_type: string; content: string }, replyTo: string | undefined): Promise<void> {
  ...
  private dispatch(event: ReceiveMessageEvent): void {
    ...
    if (isGroup && (message.mentions ?? []).length === 0) return
    let text = ''
    try {
    ...
    if (isGroup) text = text.replace(/@_user_\d+/g, '').trim()
 ...
 */
class FeishuTurnSink implements TurnSink {
  private messageId: string | undefined
  /** CardKit card entity id for the 'stream' strategy. */
  private cardId: string | undefined
  private strategy: 'stream' | 'card' | 'text' = 'stream'
  private view = '⏳ 已收到，正在处理…'
  private timer: NodeJS.Timeout | undefined
  private liveBroken = false
  private finished = false
  private lastPatched: string | undefined
  ...
  constructor(private readonly client: Lark.Client, private readonly target: ReplyTarget, private readonly log: (line: string) => void) {}

  async start(mode: TurnMode): Promise<void> {
    ...
    try {
      ...
      const cardId = (cardResp?.data as { card_id?: string } | undefined)?.card_id
      if (cardId === undefined) throw new Error(`cardkit create code=${cardResp?.code ?? '?'} ${cardResp?.msg ?? ''}`)
      ...
      if (messageId === undefined) throw new Error(`feishu send cardkit card code=${msgResp?.code ?? '?'} ${msgResp?.msg ?? ''}`)
      this.cardId = cardId
    ...
    try {
      ...
      if (id === undefined) throw new Error(`feishu create card resp missing message_id: code=${resp?.code ?? '?'} ${resp?.msg ?? ''}`)
      this.messageId = id
      this.strategy = 'card'
    } catch (error) {
      ...
      const id = resp?.data?.message_id
      if (id === undefined) throw new Error(`feishu create text resp missing message_id: code=${resp?.code ?? '?'} ${resp?.msg ?? ''}`)
      this.messageId = id
      this.strategy = 'text'
    }
    this.timer = setInterval(() => { void this.flushLive() }, PATCH_INTERVAL_MS)
  ...
  async finish(final: { text: string; markdown?: boolean }): Promise<void> {
    ...
    try {
      if (this.strategy === 'stream') {
        ...
        for (let i = 1; i < chunks.length; i++) await this.createCard(chunks[i])
  ...
  private async flushLive(): Promise<void> {
    if (this.finished || this.liveBroken || this.patchBusy) return
    if (this.strategy === 'stream') {
      const view = capView(this.view, STREAM_VIEW_CAP)
      if (view === this.lastPatched || this.cardId === undefined) return
      this.patchBusy = true
      try {
    ...
    this.patchBusy = true
    try {
      if (this.strategy === 'card') await this.patchCard(messageId, view)
      else await this.editText(messageId, view)
      this.lastPatched = view
      this.patchCount++
    } catch (error) {
      this.liveBroken = true
  ...
  private async pushStreamContent(cardId: string, content: string): Promise<void> {
  ...
  private async closeStreaming(cardId: string): Promise<void> {
    try {
  ...
  private async patchCard(messageId: string, content: string): Promise<void> {
    const resp = await this.client.im.v1.message.patch({
      path: { message_id: messageId },
  ...
  private async editText(messageId: string, text: string): Promise<void> {
  ...
  private async createCard(content: string): Promise<void> {
    const resp = await this.client.im.v1.message.create({
      params: { receive_id_type: 'chat_id' },
  ...
  private async sendFinalRaw(final: { text: string; markdown?: boolean }): Promise<void> {
...
function streamingCard(content: string): string {
...
function markdownCard(content: string): string {
  return JSON.stringify({
    config: { wide_screen_mode: true },
...
function assertOk(resp: { code?: number | undefined; msg?: string | undefined } | undefined, what: string): void {
...
function capView(view: string, cap: number): string {
  if (view.length <= cap) return view
  return `…\n${view.slice(view.length - cap + 2)}`
}

/** Split long final content on newline boundaries near the limit. */
function splitForBoundary(text: string, limit: number): string[] {
