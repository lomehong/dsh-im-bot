 ...
 */
async function validateCredentials(botId: string, secret: string): Promise<boolean> {
  if (!botId || !secret) return false
  if (botId.length < 2) return false
  if (secret.length < 2) return false
  return true
}
 ...
 * 清除企业微信凭据
 */
export async function clearWecomCredentials(): Promise<void> {
 ...
 */
export async function beginWecomLogin(session: QrLoginBridge): Promise<void> {
