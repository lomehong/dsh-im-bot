...
export class FeishuChannel {
    ...
    onCardAction(data) {
        ...
        if (token === undefined || decision === undefined || operator.open_id === undefined)
            return;
        const messageId = typeof data.open_message_id === 'string' ? data.open_message_id : undefined;
        const settleCard = async (outcome) => {
            if (this.client === undefined || messageId === undefined)
                return;
            try {
...
function approvalCardJson(card) {
    ...
    const button = (text, style, decision) => ({
        tag: 'button',
