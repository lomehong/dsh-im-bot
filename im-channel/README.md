...
| 微信 | 插件页扫码（iLink 协议） | `~/.dsh/im-channel/credentials/wechat.json` |
| 企业微信 | 插件页填 BotID + Secret（智能机器人） | `~/.dsh/im-channel/credentials/wecom.json` |

MCP 服务器（streamable-http / stdio 双传输）配置在 `~/.dsh/im-channel/credentials/mcp-servers.json`，
插件配置页可视化管理（含粘贴导入、连接测试、行内编辑请求头）。协议层基于官方 `@modelcontextprotocol/sdk`，工具以
...
- 访客可用的**工具**与**命令**由 Owner 在插件配置页「访客权限」配置（工具默认全部禁用；
