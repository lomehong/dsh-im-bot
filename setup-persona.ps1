...
# 4. persona-guide（如果不存在则创建）
$guideDir = Join-Path $PackagesDir "dsh-persona-guide"
if (Test-Path $guideDir) {
    Write-Info "构建 persona-guide..."
    Push-Location $guideDir
    npm install 2>&1 | Out-Null
    npx tsc -b tsconfig.json 2>&1 | Out-Null
    if (Test-Path "scripts/build-client.mjs") {
        node scripts/build-client.mjs 2>&1 | Out-Null
    ...
    Pop-Location
    Write-OK "persona-guide 构建完成"
} else {
    Write-Info "persona-guide 不存在，跳过（不影响核心功能）"
    Write-Warn "分身指引 Tab 将不可用，如需使用请手动创建 dsh-persona-guide 插件"
}
...
# 如果 persona-guide 存在，加入
if (Test-Path $guideDir) {
    $packageJson.dependencies["@dsh-extra/dsh-persona-guide"] = "file:$guideDir"
    $packageJson.dsh.profile.bundles += "@dsh-extra/dsh-persona-guide"
}
...
# docs
$docsDir = Join-Path $PackagesDir "docs"
New-Item -ItemType Directory -Path $docsDir -Force | Out-Null
Write-OK "文档目录已创建: $docsDir"
...
Write-Host "文档目录: $docsDir" -ForegroundColor Gray
Write-Host ""